...
<?php
include 'config.php' ;
echo "<center><font face='Comic Sans MS' size=2>Sleeping Moles from Run 1060 5-Jul 2010.</font></centre>" ;
...
$sql = 'select hasher, name, runs, hared from current where class="A1" and status="R" order by class, runs DESC limit 0,200' ;
...
$sql = 'select hasher, name, runs, hared from current where class="B1" and status="R" order by class, runs DESC limit 0,200' ;
$dbq = mysql_query($sql,$dbc) ;
echo "<table border=1 frame=box rules=all width=65%>" ;
...
$sql = 'select hasher, name, runs, hared from current where class="C1" and status="R" order by class, runs DESC limit 0,200' ;
$dbq = mysql_query($sql,$dbc) ;
echo "<table border=1 frame=box rules=all width=65%>" ;
...
$sql = 'select hasher, name, runs, hared from current where class="D1" and status="R" order by class, runs DESC limit 0,200' ;
$dbq = mysql_query($sql,$dbc) ;
echo "<table border=1 frame=box rules=all width=65%>" ;
