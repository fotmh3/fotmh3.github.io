...
$sql = 'select hasher, runs, hared from current where class="D1" order by class, runs DESC limit 0,200' ;
$dbq = mysql_query($sql,$dbc) ;
echo "<table border=1 frame=box rules=all width=50%>" ;
echo "<tr><b><th style='height:25px' width=50%>HOUNDS</th><th width=20%>RUNS</th><th width=30%>HARED RUNS</th></b></tr>" ;
